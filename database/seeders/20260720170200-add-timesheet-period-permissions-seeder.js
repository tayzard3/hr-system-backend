...
const { TIMESHEET_PERIOD_PERMISSION } = require('../constants')

/** @type {import('sequelize-cli').Migration} */
module.exports = {
	async up(queryInterface, Sequelize) {
		const newTimesheetPeriodPermissions = [
			TIMESHEET_PERIOD_PERMISSION.LIST,
			TIMESHEET_PERIOD_PERMISSION.CREATE,
			TIMESHEET_PERIOD_PERMISSION.LOCK,
			TIMESHEET_PERIOD_PERMISSION.UNLOCK,
			TIMESHEET_PERIOD_PERMISSION.DELETE,
		]
		...
		const existingPermissions = await queryInterface.sequelize.query(
			'SELECT name FROM permissions WHERE name IN (:permissions)',
			{
				...
				replacements: { permissions: newTimesheetPeriodPermissions },
			}
		)
		...
		const newPermissions = newTimesheetPeriodPermissions.filter(
	...
	async down(queryInterface, Sequelize) {
		...
		await queryInterface.bulkDelete('permissions', {
			name: newTimesheetPeriodPermissions,
		})
	},
}
